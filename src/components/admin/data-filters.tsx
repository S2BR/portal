 ...
 */
export function DataFilters({ fields }: { fields: FilterFieldDef[] }) {
  const t = useTranslations("filters");
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();

  const quickFields = fields.filter((field) => field.quick && field.type === "select");
  const quick = new Set(quickFields.map((field) => field.name));

  const [query, setQuery] = useState<FilterQuery>(() =>
    withIds(fromApiParams(filterParamsOf(new URLSearchParams(searchParams.toString())))) as FilterQuery,
  );
  ...
  useEffect(() => {
    const incoming = filterParamsOf(new URLSearchParams(searchParams.toString())).toString();
    if (incoming !== appliedRef.current) {
      appliedRef.current = incoming;
      setQuery(withIds(fromApiParams(new URLSearchParams(incoming))) as FilterQuery);
    }
  }, [searchParams]);
  ...
  const showAdvanced =
    quickFields.length === 0 || (advanced === null ? !canSimple : advanced || !canSimple);
      ...
      <div className="flex flex-wrap items-center gap-2">
        {quickFields.map((field) => (
          <FilterMultiSelect
            ...
            onChange={(values) => apply(setSimpleField(query, field.name, values))}
