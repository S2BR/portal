...
export function ProfileCategories({ categories }: { categories: Category[] }) {
  ...
  return (
    <p className="text-muted-foreground text-sm">
      {inline.map((category) => category.name).join(" · ")}
      {hasMore ? (
        <>
          {" · "}
          <Popover>
            <PopoverTrigger asChild>
              <button
                type="button"
                className="text-foreground/80 hover:text-foreground focus-visible:ring-ring cursor-pointer rounded font-medium underline-offset-2 hover:underline focus-visible:ring-2 focus-visible:outline-none"
              >
                {hiddenTopLevel > 0
